// Original code taken from the PCL/OpenNI tutorial in
...
// when pressing SPACE.
// Additionally you can take a selfie.
// To start the timer use "-t SECOND" option and press SPACE when you'r ready.
...
void *timer(void*)
{
	boost::this_thread::sleep(boost::posix_time::seconds(timeCounter));
	saveCloud = true ;
}
...
printUsage(const char* programName)
{
	cout << "Usage: " << programName << " [options]"
		 ...
		 << "\t-h         shows this help.\n"
		 << "\t-t SECOND	timer for capturing in second. Default = 0\n";
}
...
main(int argc, char** argv)
{
	...
	else if (console::find_argument(argc, argv, "-t") >= 0)
	{
		if (argc != 3);
 		else
			timeCounter = atoi(argv[2]);
	}
