...
boost::shared_ptr<pcl::visualization::PCLVisualizer> customColourVis(
	...
	viewer->addPointCloud < pcl::PointXYZ
			> (cloud, single_color, "sample cloud");
	viewer->setPointCloudRenderingProperties(
			pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "sample cloud");
	viewer->addCoordinateSystem(1.0);
	viewer->initCameraParameters();
...
boost::shared_ptr<pcl::visualization::PCLVisualizer> shapesVis(
	...
	viewer->addLine < pcl::PointXYZRGB
			> (cloud->points[0], cloud->points[cloud->size() - 1], "line");
...
boost::shared_ptr<pcl::visualization::PCLVisualizer> viewportsVis(
	...
	pcl::visualization::PointCloudColorHandlerCustom < pcl::PointXYZRGB
			> single_color(cloud, 0, 255, 0);
	viewer->addPointCloud < pcl::PointXYZRGB
			> (cloud, single_color, "sample cloud2", v2);
...
void mouseEventOccurred(const pcl::visualization::MouseEvent &event,
	...
	if (event.getButton() == pcl::visualization::MouseEvent::LeftButton
		...
		switch (counter) {
		case 0:
			viewer->addText("gogus-sol", event.getX(), event.getY(), str);
			counter++;
			ssy << xx;
			ssy << " ";
			ssy << yy;
			...
			break;
		case 1:
			viewer->addText("gogus-sag", event.getX(), event.getY(), str);
			counter++;
			ssy << xx;
			ssy << " ";
			ssy << yy;
			...
			break;
		case 2:
			viewer->addText("bel-sol", event.getX(), event.getY(), str);
			counter++;
			ssy << xx;
			ssy << " ";
			ssy << yy;
		...
		case 3:
			viewer->addText("bel-sag", event.getX(), event.getY(), str);
			counter++;
			ssy << xx;
			ssy << " ";
			ssy << yy;
			...
			break;
		case 4:
			viewer->addText("basen-sag", event.getX(), event.getY(), str);
			counter++;
			ssy << xx;
			ssy << " ";
			ssy << yy;
		...
		default:
			str_coor += ssy.str();
			cout << "sadass " << str_coor << endl;
		}
	}
}

void pointPickingEvent(const pcl::visualization::PointPickingEvent &event,
		void*viewer_void) {
	event.getPoint(xx, yy, zz);
	//cout<<xx<<"burda"<< yy<< "lan"<<zz; 
}
...
int main(int argc, char** argv) {
	io::loadPCDFile < PointXYZRGB > ("file.pcd", *cloudptr);
	// --------------------------------------
	...
	bool simple(false), rgb(false), custom_c(false), normals(false), shapes(
			false), viewports(false), interaction_customization(false);
	...
	if (simple) {
		...
		viewer = normalsVis(cloudptr, cloud_normals2);
	} else if (shapes) {
